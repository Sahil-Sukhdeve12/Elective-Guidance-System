...
const Domain = () => {
  ...
  // Fetch tracks related to the user's department and categoryId
  useEffect(() => {
    if (!userDepartment) return;

    const fetchTracks = async () => {
      try {
        console.log("Fetching tracks for categoryId:", categoryId);  // Debugging
        const response = await axios.get(`http://localhost:5000/tracks/${categoryId}`);
        const allTracks = response.data;

        console.log("Fetched tracks:", allTracks);  // Debugging
        ...
        console.log("Valid track IDs for user's department:", validTrackIds);  // Debugging
        ...
        console.log("Filtered tracks:", filteredTracks);  // Debugging
